...
function marcar_ordenacao_campo(coluna) {
    var order_crescente = true,
        order = coluna.find('i');

    if (order.length > 0) {
        order_crescente = order.hasClass('glyphicon-arrow-down');
        if (order_crescente) {
            order.removeClass('glyphicon-arrow-down');
            order.addClass('glyphicon glyphicon-arrow-up');
        }
        else {
...
function obter_order_grid() {
    ...
    colunas_grid.each(function (index, item) {
        ...
        if (order.length > 0) {
            order_crescente = order.hasClass('glyphicon-arrow-down');
            ret = coluna.attr('data-campo') + (order_crescente ? '' : ' desc');
            return true;
        }
    });
...
function abrir_form(dados) {
    ...
    bootbox.dialog({
        title: 'Regastro de ' + tituloPagina,
...
.on('click', '.page-item', function () {
    var order = obter_order_grid(),
        btn = $(this),
        filtro = $('#txt_filtro'),
        tamPag = $('#ddl_tam_pag').val(),
        page = btn.text(),
        url = url_page_click,
        param = { 'page': page, 'tamPag': tamPag, 'filtro': filtro.val(), 'order': order };
